...
function addEntry() {
  ...
  fieldIds.forEach(function(id) {
    ...
    if (id === "serviceProvided" && value === "Other") {
      value = document.getElementById("serviceOther").value;
    }
  ...
  // Hide referral site group after reset
  document.getElementById("referralSiteGroup").style.display = "none";
}

function editEntry(row) {
  // Field IDs in the same order as the table (without the Actions column)
  let fieldIds = [
    "phoneNumber", "date", "time", "callerName", "age", "sex", "location",
    "representative", "serviceProvided", "referral", "referralSite", "consultant",
    "summary", "agentName", "callModality"
  ...
  fieldIds.forEach(function(id, index) {
    ...
    if (id === "serviceProvided") {
      // If the value is one of the preset options, set the select directly;
      // otherwise, set to "Other" and populate the serviceOther field.
  ...
  // Remove the row so that when the form is submitted, the updated entry is added
  row.remove();
}

// Show/hide the "serviceOther" field based on the selected option
...
// Show/hide the "referralSite" group based on the "referral" select value
...
// Export to PDF using html2canvas and jsPDF with full alignment
function exportToPDF() {
  // Clone the table container to remove the Actions column for export
  ...
  html2canvas(clone, {
    ...
    // Calculate scaling to fit the clone on the PDF page
